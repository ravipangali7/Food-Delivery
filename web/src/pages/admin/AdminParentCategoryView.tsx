...
export default function AdminParentCategoryView() {
  const { id } = useParams();
  const { token } = useAuth();
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Parent category</p>
      ...
      {category.image_url ? (
        <img
          ...
          alt=""
          className="w-full max-h-64 object-cover rounded-xl border object-center"
        />
      ) : (
      ...
      <div className="flex gap-3">
        <Link
          to={`/admin/parent-categories/${id}/edit`}
          className="inline-block px-6 py-2 bg-primary text-primary-foreground rounded-lg text-sm"
        >
          Edit
        </Link>
        <Link to="/admin/categories/parents" className="inline-block px-6 py-2 border border-border rounded-lg text-sm">
          Back to list
        </Link>
      </div>
    </div>
